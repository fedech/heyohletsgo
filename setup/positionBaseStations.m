function [macroPos, microPos, h, clusters] = positionBaseStations (maBS, miBS, Param)
    ...
    case 'clusterized'
        ...
        for (i = 1 : Param.numClusters),
            ...
            while (~valid),
                ...
                valid = true;
                for (j = 1 : i - 1),
                    if (sqrt((x - clusters(j, 1)) ^ 2 + (y - clusters(j, 2)) ^ 2) < 100),
                        valid = false;
                    end
                end
            end
            ...
            for (j = 1 : perCluster),
                ...
                while (~valid),
                    ...
                    for (k = 1 : j - 1 + perCluster * (i - 1)),
                        if (sqrt((x - microPos(k, 1)) ^ 2 + (y - microPos(k, 2)) ^ 2) < 20),
                            valid = false;
                        end
                    end
                end
